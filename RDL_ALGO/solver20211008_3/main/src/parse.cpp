...
/**
 * @brief 为每个net分配pin, 2端口，3端口net，一次遍历signal所在chiplet内pin或IO的位置，取hdwl最小的2个或3个pin， 作为net连接的pin
 *     重构：net所连接pin确定，不需要分配，该功能可以作废
 * @param vec_chiplet  chiplet个数集合
 * @param vec_net  signal net的集合，包含所有net
 * @param IOs_parse  chiplets之间布线完成后连接到的外部IO
 */
...
void output_csv_files(vector<Edge *> vec_edge, vector<Node *> vec_node, int num_src, int demand_supply)
{
    ...
    */
    arcFrom.open("../input/Demo_Data/arcFrom.csv");
    if (!arcFrom.is_open())
        cout << "error: arcFrom.csv is not opened." << endl;
    arcFrom << "FROM_ID,Padding\n";
    arcMax.open("../input/Demo_Data/arcMax.csv");
    arcMax << "CAPACITY,Padding\n";
    arcMin.open("../input/Demo_Data/arcMin.csv");
    arcMin << "LOWER,Padding\n";
    arcTo.open("../input/Demo_Data/arcTo.csv");
    arcTo << "TO_ID,Padding\n";
    arcWeight.open("../input/Demo_Data/arcWeight.csv");
    arcWeight << "COST,Padding\n";
    nodeID.open("../input/Demo_Data/nodeID.csv");
    nodeID << "NODE_ID,Padding\n";
    nodeSD.open("../input/Demo_Data/nodeSD.csv");
    nodeSD << "SUPPLY,Padding\n";
    node_arc_Num.open("../input/Demo_Data/nodeNum_arcNum.csv");
    node_arc_Num << "nodeNum,arcNum\n";
    ...
    */
    cout << "[file output]"
         << "../input/Demo_Data/arcFrom.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/arcMax.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/arcMin.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/arcTo.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/arcWeight.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/nodeID.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/nodeSD.csv" << endl;
    cout << "[file output]"
         << "../input/Demo_Data/nodeNum_arcNum.csv" << endl;
    arcFrom.close();
    arcMax.close();
    arcMin.close();
    arcTo.close();
...
void output_routing_path(vector<vector<Coord3 *>> vec_coord3_uncleaned, vector<BP *> allocated_bp, string Filename, int W_FP, int H_FP, int pad_size, vector<vector<routePoint>> &routePoints)
{
    ofstream outfile;
    outfile.open(Filename);
    if (!outfile.is_open())
    ...
    for (size_t i = 0; i < vec_coord3_uncleaned.size(); i++)
    {
                        ...
                        if (is_bp)
                        {
                            ...
                            if (new_coord3_IOpad->is_straitline(front_coord, back_coord))
                            {
                                ...
                                output_coord3(outfile, new_coord3_IOpad);
                                    ...
                                    routePoint rPointB;
                                    rPointB.x = back_coord->x;
                                    rPointB.y = back_coord->y;
                                    rPointB.h = back_coord->h;
                                    vecPoints.push_back(rPointB);
                                    routePoint rPointN;
                                    rPointN.x = new_coord3_IOpad->x;
                                    rPointN.y = new_coord3_IOpad->y;
                                    rPointN.h = new_coord3_IOpad->h;
                                    vecPoints.push_back(rPointN);
                            ...
                            else
                            {
                                ...
                                output_coord3(outfile, new_coord3_IOpad);
                                    ...
                                    vecPoints.push_back(rPointB);
                                    routePoint rPointF;
                                    rPointF.x = front_coord->x;
                                    rPointF.y = front_coord->y;
                                    rPointF.h = front_coord->h;
                                    vecPoints.push_back(rPointF);
                                    routePoint rPointN;
                                    rPointN.x = new_coord3_IOpad->x;
                                    rPointN.y = new_coord3_IOpad->y;
                                    rPointN.h = new_coord3_IOpad->h;
                        ...
                        if (is_bp)
                        {
                            ...
                            if (new_coord3_IOpad->is_straitline(front_coord, mid_coord))
                            {
                                ...
                                output_coord3(outfile, new_coord3_IOpad);
                                    ...
                                    vecPoints.push_back(rPointB);
                                    routePoint rPointM;
                                    rPointM.x = mid_coord->x;
                                    rPointM.y = mid_coord->y;
                                    rPointM.h = mid_coord->h;
                                    vecPoints.push_back(rPointM);
                                    routePoint rPointN;
                                    rPointN.x = new_coord3_IOpad->x;
                                    rPointN.y = new_coord3_IOpad->y;
                                    rPointN.h = new_coord3_IOpad->h;
        ...
        routePoints.push_back(vecPoints);
    }
}
...
void output_3_pin_nets_mr3(vector<Net *> vec_net, int wire_spacing, string filename_maze_route_output, vector<vector<routePoint>> &routePoints)
{
    ofstream outfile;
    outfile.open(filename_maze_route_output);
    if (!(outfile.is_open()))
    ...
    for (size_t i = 0; i < vec_net.size(); i++)
    {
        ...
        for (size_t j = 0; j < temp_net->used_mbs.size(); j++)
        {
            ...
            if (temp_mb_prev != nullptr)
            {
                ...
                if (!(temp_mb->is_neighbor(temp_mb_prev, wire_spacing)))
                {
                    routePoints.push_back(vecPoint);//
                    vecPoint.resize(0);
                    outfile << endl;
                }
            }
            routePoint rPoint;//
            rPoint.x = temp_mb->coord_mid->x;//
            rPoint.y = temp_mb->coord_mid->y;//
            rPoint.h = temp_mb->idx_h;//
            vecPoint.push_back(rPoint);//
            outfile << "(" << temp_mb->coord_mid->x << ", " << temp_mb->coord_mid->y << ", " << temp_mb->idx_h << ")";
            // if (j != temp_net->used_mbs.size() - 1)
            //     outfile << " - ";
...
void output_solution_to_json_2(const string &input_json_filename, vector<Net *> vec_net, vector<IO *> vec_IO)
{
    ifstream infile(input_json_filename);
    json inJson;
    infile >> inJson;
