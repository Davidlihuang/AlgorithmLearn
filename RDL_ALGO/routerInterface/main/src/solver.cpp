#include "solver.h"
void Solver::parsing_from_json(const DbWrapper::DbWrapper &dbWrapper, const string &strJsonFilename)
//void Solver::parsing_from_json(const string &strJsonFilename)
{
    ifstream inFile(strJsonFilename);
    json myJson;
    inFile >> myJson;
    //parse floorplan_size
    ////W_FP = myJson["FP_Size"][0];
    ////H_FP = myJson["FP_Size"][1];
    W_FP = dbWrapper.m_grid.nMaxX;
    H_FP = dbWrapper.m_grid.nMaxY;
    //cout << "dbWrapper " << dbWrapper.m_grid.nMinX << " " << dbWrapper.m_grid.nMinY << " " << dbWrapper.m_grid.nMaxX << " " << dbWrapper.m_grid.nMaxY <<  " " << dbWrapper.m_grid.nOffsetDistance << endl;

    //parse Chiplet names
    ////for (size_t i = 0; i < myJson["Chiplets"].size(); i++)
    for (size_t i = 0; i < dbWrapper.m_instPins.size(); i++)
    {
        ////string new_chiplet_name = myJson["Chiplets"][i]["Name"];
        string new_chiplet_name = dbWrapper.m_instPins[i].struInstance.strInstName;
        Chiplet *new_chiplet = new Chiplet(new_chiplet_name);
        chiplets.push_back(new_chiplet);
        //cout << "InstName " << new_chiplet_name << " " << dbWrapper.m_instPins[i].struInstance.strInstName<< endl;
    }
    ...
    for (size_t i = 0; i < dbWrapper.m_instPins.size(); i++)
    {
        ...
        int w = dbWrapper.m_instPins[i].struInstance.nInstWidth, h = dbWrapper.m_instPins[i].struInstance.nInstHeight;
        chiplets[i]->width = w;
        chiplets[i]->height = h;
    }
    ...
    for (size_t i = 0; i < dbWrapper.m_instPins.size(); i++)
    {
        ...
        int x = dbWrapper.m_instPins[i].struInstance.nInstX, y = dbWrapper.m_instPins[i].struInstance.nInstY;
        chiplets[i]->x = x;
        chiplets[i]->y = y;
    ...
    for (size_t i = 0; i < dbWrapper.m_instPins.size(); i++)
    {
        ...
        for (size_t j = 0; j < dbWrapper.m_instPins[i].vecPinLocations.size(); j++)
        {
            ...
            int x = dbWrapper.m_instPins[i].vecPinLocations[j].nPosX;
            int y = dbWrapper.m_instPins[i].vecPinLocations[j].nPosY;
    ...
    for (size_t i = 0; i < dbWrapper.m_instPins.size(); i++)
    {
        ...
        for (size_t j = 0; j < dbWrapper.m_instPins[i].vecPinNames.size(); j++)
        {
            ////string new_pin_name = myJson["Chiplets"][i]["Pin_Names"][j];
            string new_pin_name = dbWrapper.m_instPins[i].vecPinNames[j];
    ...
    for (size_t i = 0; i < dbWrapper.m_ioPins.vecPinNames.size(); i++)
    {
        //cout << myJson["Ext_Ports"][i] << endl;
        ////string new_io_name = myJson["Ext_Ports"][i];
        string new_io_name = dbWrapper.m_ioPins.vecPinNames[i];
    ...
    IOs_parse->pad_size = dbWrapper.m_ioPins.nPadWidth;
    IOs_parse->pin_size = dbWrapper.m_ioPins.nPinWidth;

    //parse netnames and pins
    ////for (auto &x : myJson["Netlist"].items())
    for (size_t t = 0; t < dbWrapper.m_nets.size(); t++)
    {

        //cout << "netname: " << x.key() << ", value: " << x.value() << '\n';
        ////Net *new_net = new Net(x.key());
        Net *new_net = new Net(dbWrapper.m_nets[t].strNetName);
        new_net->delay = MAX_NUM_INT;
        //std::cout << "new net name:" << x.key() << endl;

        ////for (size_t i = 0; i < x.value().size(); i++)
        for (size_t i = 0; i < dbWrapper.m_nets[t].vecConnectNames.size(); i++)
        {
            ...
            for (size_t j = 0; j < IOs_parse->IOnames.size(); j++)
            {
                ...
                if (dbWrapper.m_nets[t].vecConnectNames[i] == IOs_parse->IOnames[j])
                {
                    new_net->is_IO_connection = true;
                    ////Pin *new_pin = new Pin(x.value()[i], chiplets.size()); //use the last index as IO_parse index
                    Pin *new_pin = new Pin(dbWrapper.m_nets[t].vecConnectNames[i], chiplets.size()); //use the last index as IO_parse index
            ...
            for (size_t j = 0; j < chiplets.size(); j++)
            {
                ...
                for (size_t k = 0; k < temp_chiplet->signals.size(); k++)
                {
                    ...
                    if (dbWrapper.m_nets[t].vecConnectNames[i] == temp_chiplet->signals[k])
                    {
                        idx_chiplet = j;
                        ////Pin *new_pin = new Pin(x.value()[i], idx_chiplet);
                        Pin *new_pin = new Pin(dbWrapper.m_nets[t].vecConnectNames[i], idx_chiplet);
                        new_net->add_new_pinname_structure(new_pin);
                        break;
                    }
                }
    ...
    string filename_wirebudget = "../output/sdc_netlist_with_ascending_delay_wirebudget.out";
    ...
    double Ext_Port_Min_Space = dbWrapper.m_ioPins.nMinSpace;
    ...
    for (size_t i = 0; i < dbWrapper.m_ioPins.vecPinLocations.size(); i++)
    {
        Coord *coord = new Coord(dbWrapper.m_ioPins.vecPinLocations[i].nPosX, dbWrapper.m_ioPins.vecPinLocations[i].nPosY);
        IOs_parse->placed_pinLocs.push_back(coord);
    }
}

//int main(int argc, char **argv) $Input_JSON -vc 2 $Input_JSON -vc 2 -rs -it 10 
void Solver::run(const DbWrapper::DbWrapper &dbWrapper, vector<vector<Coord3>> &routeData, const string &strInput,const string &strVc, int nVc, const string &strRs, const string &strIt, int nIt)
//void Solver::run(const string &strInput,const string &strVc, int nVc, const string &strRs, const string &strIt, int nIt)
{
    ...
    if (is_read_flow_solution)
    {
        ...
        //{
            if (strIt == "-it")
            {
                cout << "[max compaction iteration num] " << nIt << endl;
                iter_compaction = nIt;
            }
        //}
    }

    parsing_from_json(dbWrapper, strInput);
    //parsing_from_json(strInput);
    //sort_net_by_descending_delay(nets);
    //cout << "[sort_net_by_descending_wirebudget]" << endl;
    //sort_net_by_descending_wirebudget(nets, clock_period);
    ...
    // for (size_t i = 0; i < nets_3pn_dummy_2pn.size(); i++)
    // {
    //     cout << nets_3pn_dummy_2pn[i]->netname << ": " << nets_3pn_dummy_2pn[i]->coord_pins.size() << " chiplet combination: " << nets_3pn_dummy_2pn[i]->pinnames_structure[0]->idx_chiplet
    //          << ", " << nets_3pn_dummy_2pn[i]->pinnames_structure[1]->idx_chiplet << endl;
    // }
    // for (size_t i = 0; i < nets.size(); i++)
    // {
    //     cout << nets[i]->netname << ": " << nets[i]->coord_pins.size() << " pins" << endl;
    // }
    ...
    //maze route solver
    //bool found_solution = maze_route(nets, maze_blocks, allocated_bp);
    // bool found_solution_random_maze = false;
    // if (!found_solution)
    // {
    //     float routability = 0;
    //     float max_routability = 0;
    //     unsigned int seed_max_routability;
    //     while (!found_solution_random_maze)
    //     {
    //         unsigned int seed = unsigned(time(0));
    //         srand(seed);
    //         maze_blocks_randomized = gen_maze_blocks(tiles, numRDL);
    //         //print_blocked_mazeblocks(maze_blocks);
    //         map_IO_to_mazeblock(IOs, maze_blocks_randomized);
    //         found_solution_random_maze = randomized_maze_route(nets, maze_blocks_randomized, allocated_bp, routability);
    //         if (routability > max_routability)
    //         {
    //             max_routability = routability;
    //             seed_max_routability = seed;
    //             cout << " seed: " << seed;
    //         }
    //         cout << " max_routability: " << max_routability << " with seed: " << seed_max_routability << endl;
    //     }

    //     print_blocked_mazeblocks(maze_blocks_randomized);
    //     cout << "sucess: found a solution" << endl;
    // }
    //

    //attach_tiles_to_bp(tiles, nodes);
    ...
    string filename_output_maze_route = "../output/maze_route_multipin.txt";
    ...
    if (!is_read_flow_solution)
    {
        ...
        {
            vector<Coord3> vecCoord;
            for (size_t j = 0; j < routePoints[i].size(); j++)
            {
                Coord3 coord(0,0,0);
                coord.x = routePoints[i][j].x;
                coord.y = routePoints[i][j].y;
                coord.h = routePoints[i][j].h;
                vecCoord.push_back(coord);
            }
            routeData.push_back(vecCoord);
        }
    }
    ...
    outfile_3_pin_nets_res.open("../output/res_used_by_3_pin_nets.txt");
    ...
    if (is_read_flow_solution)
    {
        ...
        string output_routing_path_file_uncompact = "../output/routing_path_2pin_nets_uncompact.txt";
        ...
        string output_routing_path_file_compact = "../output/routing_path_2pin_nets_compact.txt";
        vector<vector<routePoint>> routePoints;
        output_routing_path(vec_coord3_set_uncleaned_2, allocated_bp, output_routing_path_file_compact, W_FP, H_FP, IO_pad_size, routePoints); // for python
        cout << "routePoints " << routePoints.size() << endl;
        for (size_t i = 0; i < routePoints.size(); i++)
        {
            vector<Coord3> vecCoord;
            for (size_t j = 0; j < routePoints[i].size(); j++)
        ...
        output_solution_to_json_2(strInput, nets, IOs);
