...
using namespace std;
//using namespace std::chrono;Ford-Fulkerson
...
void parsing_from_json(char *filename)
{
    ...
    for (size_t i = 0; i < myJson["Chiplets"].size(); i++)
    {
        ...
        for (size_t j = 0; j < myJson["Chiplets"][i]["Pin_Locations"].size(); j++)
        {
            ...
            ///< pin_locations的pin坐标是相对于chiplet坐下角坐标定的，因此计算pin在global中的位置必须加上chiplet左下角的坐标
    ...
    for (auto &x : myJson["Netlist"].items())
    {
        ...
        for (size_t i = 0; i < x.value().size(); i++)
        {
            ...
            ///< 获取单芯片的对外port端口， 分配id ： chiplet.size()
    ...
    for (int i = 0; i < numIO_horizontal; i++)
    {
        ...
        double x = (double)bases_x + 0.5 * (double)unit_spacing_horizontal + (double)i * (double)unit_spacing_horizontal;   ///< 每一个pin矩形的中心点
        double lower_y = 0.5 * 50; 
        double upper_y = H_FP - 0.5 * 50;
        Coord *new_coord_lower = new Coord(x, lower_y);   ///< 芯片底部IO的所有pin的图形的中心坐标
        Coord *new_coord_upper = new Coord(x, upper_y);  ///< 芯片顶部IO的所有pin的图形的中心坐标
        IOs_parse->placed_pinLocs.push_back(new_coord_lower); ///<  将最外层封装的PIN坐标保存起来
...
int main(int argc, char **argv)
{
   ...
   ///<@todo 没看懂
    ...
    ///<@todo 链接3个pin的net，增加两个dummy的pin
    ...
    y_OB = Offset_Distance + IO_pad_size;///< boundary边界中构建了很多以线宽大小的迷宫小室。 是一个平面的二维表示
    tiles = gen_tile_vector(x_OB, y_OB, W_OB, H_OB, wire_spacing, chiplets, nodes, edges, W_OB / wire_spacing, H_OB / wire_spacing);  

    IOs = gen_IOs(IOs_parse, W_FP, H_FP); //< 被使用到的pin
    routing_track_io_pad = gen_routing_track_for_io_pad(x_OB, y_OB, W_OB, H_OB, W_FP, H_FP, IO_pad_size);
    //print_io_tracks(routing_track_io_pad); //debug

    IOs_parse->gen_available_bp(tiles);   ///< 将是边界的tile坐标保存在IOs_parse中
    route_to_bp(IOs, routing_track_io_pad, IOs_parse, x_OB, y_OB, W_FP, H_FP, allocated_bp); ///< block point
    ...
    //Print blocked m打印mazeazeblock layout 打印maze 图的布局
    print_blocked_mazeblocks(maze_blocks); //debug
    ...
    if (!is_read_flow_solution)
    {
        ...
        maze_route_for_3plus_pins_mr3(nets, maze_blocks, allocated_bp); ///< BFS迷宫算法对3+pin net进行布线
        output_3_pin_nets_mr3(nets, wire_spacing, filename_output_maze_route); ///< 每条net上的路径坐标输出到文件中，供可视化使用
        //print_blocked_mazeblocks(maze_blocks);
    }
    ...
    if (is_read_flow_solution)
    {
        ...
        cout << "[read_paths_into_nets]: " << path_2pin.size() << endl;
