...
public:
    ...
    double delay;
    vector<Coord *> coord_pins;   ///< 坐标有何用处

    //Path *assigned_path;

    //for maze route
    vector<Coord *> coord_pins_mr3;   ///< 保存此坐标代表什么？
...
{
public:
    int pad_size;                                              ///< IO pad的大小50        
    int pin_size;
    vector<string> IOnames;                     ///< chiplet向外引出的IO名字
    vector<Coord *> placed_pinLocs;   ///< IO的pin的物理坐标，pin矩形的中心点坐标
